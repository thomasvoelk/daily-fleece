...
The boundary is event-driven: when the Host sets the Correct Answer for a question, `quiz` publishes a scoring event carrying player IDs and their answers; `player` consumes it to update the Leaderboard document. Individual answer data stays embedded in the Session document inside `quiz` — `player` never reads it directly. `quiz` never imports `player` internals — it references players by ID only.
