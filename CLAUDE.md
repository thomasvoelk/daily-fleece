...
| User display name | `de.dailyfleece.backend.player.api.DisplayName` | carries non-blank rule; shared across modules |
...
**`DisplayName` is shared via a named interface.** It lives in `de.dailyfleece.backend.player.api` (annotated `@NamedInterface`). The quiz module declares `@ApplicationModule(allowedDependencies = "player :: api")` to access it. Do not duplicate this type in the quiz module.
